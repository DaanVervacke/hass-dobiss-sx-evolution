...
async def test_reload_listener_title_rename_updates_device_registry(
    ...
    """Renaming a module subentry must update the device name via the fast path.

    The fast path (unload + re-forward platforms) only recreates entities.
    ...
    device = device_registry.async_get_device(identifiers={identifier})
    assert device is not None
    assert device.name == "Kitchen module"
